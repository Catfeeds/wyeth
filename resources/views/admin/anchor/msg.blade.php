            ...
            <div class="user-heading alt green-bg" style="padding: 8px; position: relative;">
                ...
                <div style="position: absolute; top:0; right:0; width:50%; padding: 8px;">
                    ...
                    <div style="margin: 14px 0 4px 0;">
                        <input id="notifyNew" hw-data="0" type="button" class="btn btn-primary btn-xs" value="提醒医生回答问题" cid="{{$courseInfo->id}}">
                    </div>
                </div>
            </div>
        </section>
        ...
        <div class="panel-body profile-activity">
            <div class="tab-content tasi-tab">
                ...
                <div style="position: relative; overflow: hidden; border:1px solid #aec785; padding: 10px 10px 0px 10px; margin-bottom: 10px; border-radius: 4px;">
                    ...
                    @endforeach
                    @if ($user['hasUnSendMsg'] == 1)
                    <button type="submit" class="btn btn-info" style="position: absolute; right:10px; bottom: 10px;">发送给好医生</button>
                    @endif
                </div>
                @endforeach
                </div>
...
$(function($) {
    $('button').click(function(){
        ...
        if ($(this).attr('is_send') > 0) {
            ...
            $(this).html('发送给好医生').attr('is_send', 0);
            return ;
        }
        var msgIds = [];
        ...
        var sendMsg = function(){
            var cid = $('#cid').val();
            $.post("/admin/anchor/sendMsg", {msgIds: msgIds},
            function (data) {
                if (data == 1) {
        ...
        $(this).html('提交中，点击取消').attr('is_send', setTimeout(sendMsg, 2000));
    });
    $('#notifyNew').click(function(){
        var status = $('#notifyNew').attr('hw-data');
        if (status == 1) {
            alert('请不要重复提交');
            return false;
        }
