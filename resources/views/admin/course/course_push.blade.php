                                ...
                                <div class="form-group" style="display: flex;flex-direction: row-reverse">
                                    <div class="col-md-6">
                                        <div class="input-group input-large">
                                            ...
                                            <div class="input-group input-large">
                                                <select class="form-control m-bot15 col-lg-12" name="status" style="width: 130px" onchange="submitMyForm()">
                                                    @if(isset($params['status']))
                                                        @if($params['status'] == 'all')
                                                            <option value="all" selected = "selected"><?php echo $params['status_value']?></option>
                                                            <option value="normal">开课与回顾</option>
                                                            <option value="special">系统推送</option>
                                                        @elseif($params['status'] == 'normal')
                                                            <option value="normal" selected = "selected"><?php echo $params['status_value']?></option>
                                                            <option value="all">全部</option>
                                                            <option value="special">系统推送</option>
                                                        @elseif($params['status'] == 'special')
                                                            <option value="special" selected = "selected"><?php echo $params['status_value']?></option>
                                                            <option value="all">全部</option>
                                                            <option value="normal">开课与回顾</option>
                                                        @endif
                                                    @else
                                    ...
                                    @if (!empty($list))
                                        @foreach ($list as $k => $item)
                                            <tr id="push{{ $item->id }}">
                                                ...
                                                <td>
                                                    @if($user_info->user_type != 2)
                                                        <a href="/admin/course/index?id={{ $item->cid }}">{{ $item->course_name }}</a>
                                                    @else
                                                        {{ $item->course_name }}
                                                    @endif
                                                </td>
                                                <td class="push_time">{{ $item->push_time }}</td>
                                                <td class="push_num">{{ $item->push_num }}</td>
                                                <td class="remark">{{ $item->remark }}</td>
                                                @if($item->status == 0)
                                                    <td>待推送,预计推送{{ $item->excepted }}</td>
                                                @else
                                                    <td>{{ "已推送 " }}<button data-toggle="modal" data-target="#detail" class="btn btn-primary btn-xs" onclick="getDetail('{{ $item->id }}')"><i class="fa fa-info-circle "></i>详情</button></td>
                                                @endif
                                                    <td>{{ $item->created_at }}</td>
                                                @if($user_info->user_type != 2)
                                                    @if ($item->status == 0)
                                                        <td>
                                                            <button class="btn btn-primary btn-xs" data-toggle="modal" onclick="idEdit = {{ $item->id }}; clickEdit();" data-target="#editModal"><i class="fa fa-pencil"></i>编辑</button>
                                                            <button onclick="deleteConfirm({{ $item->id }})" class="btn btn-danger btn-xs"><i class="fa fa-trash-o "></i>删除</button>
                                                        </td>
                                                    @else
                                                        <td>已推送</td>
                                                    @endif
