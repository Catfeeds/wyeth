...
<script type="text/javascript">
    ...
    var user_type = 2;
    var author_id =  {{$course_info->anchor_uid}};
    var teacher_author_id =  {{$course_info->teacher_uid}};
    ...
    function wssend(data) {
        ...
        if (data.type != 'pong') {
            data = _.extend(data, {token: this.token});
        }
    ...
    function get_to_teacher_question(){
        $.post("/api/message/question", { cid:cid, userid:teacher_author_id, type:'anchor'},
            function(data){
                if (data['status'] == 1){
                    data = data['data']['list'];
                    var html = '';
    ...
    function flush_client_list(type, author_id) {
        ...
        if (type == 'login') {
            ...
            client_list_select.find('option[value="'+ author_id +'"]').remove();
        }
    }
    ...
    $("input[name='course_status']").click(function(){
        var status = $(this).val();
        $.post("/anchor/changeCourseStatus", { cid:cid, status:status },
            function(data){
                if (data == 1){
                    alert('课程状态修改成功');
                } else {
    ...
    $("input[name='speak_status']").click(function(){
        ...
        $.post("/anchor/changeCourseSpeakStatus", { cid:cid, status:status },
            function(data){
                if (data == 1){
                    alert('禁言状态修改成功');
                } else {
    ...
    $(function(){
        ...
        $('#mySwitch').on('switch-change', function (e, data) {
            ...
            $.post("/anchor/changeReplyNotifyStatus", { cid:cid, status:status },
