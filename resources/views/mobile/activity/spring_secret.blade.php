...
<head>
    ...
    <style>
        ...
        .main-body {
            ...
            @if($aid == 2)
            background: url('{{$su}}/mobile/images/activity/2/bg_aid2_v6.png') no-repeat center 0;
            @elseif($aid == 3)
            background: url('{{$su}}/mobile/images/activity/2/bg_aid3_v6.png') no-repeat center 0;
            @else
            background: url('{{$su}}/mobile/images/activity/2/bg_aid4_v6.png') no-repeat center 0;
            @endif
            @if($aid == 2)
            height: 1334px;
            @elseif($aid == 3)
        ...
        .container-voice {
            width: 650px;  height: 154px;  border-radius: 20px;  background-color: #fff; box-shadow: 2px 2px 4px #d4d4d4;  display: -webkit-flex; display: flex;  -webkit-flex-direction: row;  flex-direction: row;  -webkit-align-items: center;  -webkit-box-align: center;  align-items: center;
        }
        ...
        .text-voice-teacher, .text-voice-time, .text-voice-current, .text-voice-divider {
            color: #999;  font-size: 24px;  line-height: 24px;
        }
        .container-voice-time {
            width: 490px;  display: -webkit-flex;  display: flex;  -webkit-justify-content: space-between;  justify-content: space-between; -webkit-box-flex: 1; flex: 1; -webkit-box-align: center; -webkit-align-items: center; align-items: center;
        }
        ...
        .share_page_block1 {
            background: url('{{$su}}/mobile/images/activity/2/share_bg1_v2.png') no-repeat center 0;  width: 100%;  height: 822px;
        }
        .share_page_block2 {
            background: url('{{$su}}/mobile/images/activity/2/share_bg2_v2.png') no-repeat center 0;  width: 100%;  height: 284px;
        }
...
<div class="main-body" id="#content-container">
    ...
    @foreach($configs->courses as $index => $course)
        @if($index<3)
            <div class="container-course" style="margin-top: @if($index == 0) 71px @else 38px @endif">
                <div class="container-voice" style="border: #FAC13B 3px solid;">
                    ...
                    <div style="display: flex;flex-direction: column;height: 154px;padding: 22px 0 24px 0;">
                ...
                {{--@if($aid == 4 && $invite_num <= 1 && $index > 0)--}}
                    {{--<div style="width: 650px;height:154px;border-radius:20px;background-color: rgba(0,0,0,0.12); position: absolute; top: 0;">--}}
                        {{--<div style="width: 90px; height: 90px; border-radius: 45px; background: #FFA42F; opacity: 1; margin: 20px; padding-top: 21px; margin-top: 30px">--}}
    ...
    {{--@if($aid != 4)--}}
        @if($user->crm_status)
            <img src="{{$su}}/mobile/images/activity/2/btn_get_more_v2.png" style="width: 312px; height: 94px; margin-top: 63px" onclick="window.location.href = '/mobile/index?defaultPath=/all&forcejump=1'">
        @else
            <img src="{{$su}}/mobile/images/activity/2/btn_get_more_v2.png" style="width: 312px; height: 94px; margin-top: 63px" onclick="goRegister()">
        @endif
    {{--@else--}}
...
<div class="pop-window" id="ruleWindow">
    ...
    <div class="pop-window-rule">
        <div style="margin-top: 103px; height: 760px; overflow: scroll">
            <span class="text-rule-desc">1.早春魔栗指南一共3期，每期3节精选自魔栗妈咪学院的课程。</span>
            <span class="text-rule-desc">2.每期第1节课免费试听，第2/3节课需要邀请成功邀请2名好友注册惠氏妈妈俱乐部，即可解锁。</span>
            <span class="text-rule-desc">3.解锁当期全部课程后，转发活动页，可参与抽奖，奖品为惠氏妈妈俱乐部提供的马良行定制声波戒指。</span>
            <span class="text-rule-desc">4.任务解锁成功通知、中奖通知，将通过惠氏妈妈俱乐部模板消息另行通知，敬请持续关注惠氏妈妈俱乐部公众号。</span>
            <span class="text-rule-desc">5.活动开奖时间为2018年3月5日。</span>
            <span class="text-rule-desc">6.中奖通知发出后，请中奖者于3个工作日内填写地址和相关信息，未如期填写信息则视为自动放弃。</span>
            <span class="text-rule-desc">7.因奖品为定制首饰，需要20个工作日制作。实际奖品发放时间将于中奖通知发布时，同时公布，敬请关注。</span>
        </div>
    </div>
...
//    var scene_str = '';
    var step_texts = [
        '{{$su}}/mobile/images/activity/2/step_text_1.png',
    ...
    var step_ts = [
        '{{$su}}/mobile/images/activity/2/step_t_1.png',
...
//            if (result.ret == 1) {
                ...
                step_bg.src = '{{$su}}/mobile/images/activity/2/step_bg_v4.png';
...
//    });
    ...
    $(document).ready(function () {
        ...
        {{--if ('{{$aid}}' == 4 && '{{$invite_num}}' > 1 && (!localStorage.springInviteNum || localStorage.springInviteNum != '{{$invite_num}}')) {--}}
...
//    微信分享
    ...
    var shareTitle = 'get早春魔栗指南，宝宝好，你也好';
    var sharePicture = '{{$su}}/mobile/images/activity/2/icon_text.png';
...
<script>
    CIData.push(['trackEvent', 'wyeth', 'spring_secret_channel', 'wyeth_channel', getQueryStringByName('_hw_c')]);
</script>
</body>