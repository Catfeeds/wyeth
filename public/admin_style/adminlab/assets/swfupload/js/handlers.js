...
function swfUploadLoaded() {
	...
	//document.getElementById("btnCancel_"+this.field_key).onclick = function () { self.cancelQueue(); };
}
...
function uploadStart(file) {
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("正在上传...");//Uploading...
		progress.toggleCancel(true, this);
	}
	catch (ex) {}
...
function uploadSuccess(file, serverData) {
	try {
		...
		if(serverData.src != ''){
			show_uplad_ware(serverData.src,serverData.img_url,serverData.img_id);
		}else{
			alert('上传失败：'+serverData.msg);
			return false;
		}
		
		progress.setComplete();
		progress.setStatus("已完成.");//Complete.
		progress.toggleCancel(false);
		//alert('一个上传完成');
	} catch (ex) {
		this.debug(ex);
...
function uploadComplete(file) {
	if (this.getStats().files_queued === 0) {
		...
		$("#"+this.customSettings.progressTarget).slideUp();
	}
}
