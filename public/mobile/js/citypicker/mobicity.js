(function ($) {
    function Scroller(elem, settings) {
		...
		function changeItem(v){
			var that = this;
			s.wheels[0][this.settings.areaText] = {};
			if(type == this.settings.provinceText){
				s.wheels[0][this.settings.cityText] = {};
				for(var i = 0; i < that.settings.data.length; i++){
					if(that.settings.data[i]['value'] == this.temp[0]){
						$.each(that.settings.data[i].children, function(index){
							s.wheels[0][that.settings.cityText][this['value']] = this['text'];
							if(index == 0){
								$.each(this.children, function(){
									s.wheels[0][that.settings.areaText][this['value']] = this['text'];
								});
							}
						});
						break;
				...
				that.refresh([ this.settings.cityText, this.settings.areaText ]);
			}
			else if(type == this.settings.cityText){
				s.wheels[0][this.settings.areaText] = {};
				for(var i = 0; i < that.settings.data.length; i++){
					if(that.settings.data[i]['value'] == this.temp[0]){
						for(var j = 0; j < that.settings.data[i]["children"].length; j++){
							if(that.settings.data[i]['children'][j]['value'] == this.temp[1]){
								$.each(that.settings.data[i]['children'][j].children, function(){
									s.wheels[0][that.settings.areaText][this['value']] = this['text'];
								});
								break;
							}
				...
				that.refresh([ this.settings.areaText ]);
			}
		}
        ...
        that.change = function (manual) {
            ...
            if (manual) {
                event('onChange', [v,type]);
				if(type && type != this.settings.areaText){
					changeItem.call(that, v);
				}
            }
        };
			...
			for (i = 0; i < s.wheels.length; i++) {
                for (label in s.wheels[i]) {
					if($.inArray(label, labels) > -1){
						...
						$(".dwwl" + l + " .dw-ul").html(generateWheelItems(l));
					}
					l++;
                }
            }
        ...
        that.show = function (prevAnim) {
            ...
            for (i = 0; i < s.wheels.length; i++) {
                ...
                for (label in s.wheels[i]) {
                    warr[l] = s.wheels[i][label];
                    html += '<td><div class="dwwl dwrc dwwl' + l + '">' + (s.mode != 'scroller' ? '<div class="dwwb dwwbp" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>+</span></div><div class="dwwb dwwbm" style="height:' + hi + 'px;line-height:' + hi + 'px;"><span>&ndash;</span></div>' : '') + '<div class="dwl">' + label + '</div><div class="dww" style="height:' + (s.rows * hi) + 'px;min-width:' + s.width + 'px;max-width:' + s.width + 'px;text-overflow:ellipsis;"><div class="dw-ul">';
                    // Create wheel values
            ...
            html += (s.display != 'inline' ? '<div class="dwbc' + '"><span class="dwbw dwb-s"><span class="dwb">' + s.setText + '</span></span>' + '<span class="dwbw dwb-c"><span class="dwb">' + s.cancelText + '</span></span></div></div>' : '<div class="dwcc"></div>') + '</div></div></div>';
            dw = $(html);
        ...
        that.init = function (ss) {
			...
			//preset.call(e, that);
			pres = $.citypicker.preset.call(e, that);
    ...
    $(document).bind(END_EVENT, function (e) {
        if (move) {
			type = target.parent().prev("div.dwl").text();
    ...
    $.scroller = $.scroller || $.citypicker;
    $.fn.scroller = $.fn.scroller || $.fn.citypicker;
})(jQuery);
...
(function ($) {
	...
	ms.preset = function (inst) {
		...
		$.each(s.data, function(i){
			w[s.provinceText][this['value']] = this['text'];
		});
		$.each(s.data[0].children, function(i){
			w[s.cityText][this['value']] = this['text'];
		});
		$.each(s.data[0].children[0].children, function(i){
			w[s.areaText][this['value']] = this['text'];
		});
		wheels.push(w);
		...
		function getCity(d) {
			...
			if($.isArray(d) && d.length === 3){
				for(var k in w[s.provinceText]){
					if(k == d[0]){
						text.push(w[s.provinceText][k]);
					}
				}
				if(d[1]){
					for(var k in w[s.cityText]){
						if(k == d[1]){
							text.push(w[s.cityText][k]);
						}
					}
				}
				else{
				...
				if(d[2]){
					for(var k in w[s.areaText]){
						if(k == d[2]){
							text.push(w[s.areaText][k]);
						}
					}
				}else{
					text.push('');
		...
		inst.setCity = function (d, fill, time, temp) {
			var i;
			// Set wheels
			for (i in o) {
		...
		return {
			...
			headerText: function (v) {
				return ms.formatValue(format, getCity(inst.temp), s);
			},
			formatResult: function (d) {
				return ms.formatValue(format, getCity(d), s);
			},
			parseValue: function (val) {
				return ms.parseValue(format, val, s);
			},
			...
			methods: {
				getCity: function (temp) {
					var inst = $(this).citypicker('getInst');
					if (inst) {
						return inst.getCity(temp ? inst.temp : inst.values);
					}
				},
				setCity: function (d, fill, time, temp) {
					if (fill == undefined) {
						fill = false;
					}
					return this.each(function () {
    ...
    ms.formatValue = function (format, value, settings) {
        if (!value) {
            return null;
        }
		value = value.split('-');
		return format.replace("{p}", value[0]).replace("{c}", value[1]).replace("{a}", value[2]);
    };

    ms.parseValue = function (format, value, settings) {
        return [];
    };
