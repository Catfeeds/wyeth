AppList = {
    ...
    initSearch: function () {
        ...
        $('[hw_type]').on('click', function(e) {
            var typeCurr = $(e.currentTarget).attr('hw_type');
            self.options.dropParam.type = typeCurr;

            if (typeCurr == 'review') {
                self.options.dropParam.tag = $('[hw_tag_list_review] [hw_tag].active').attr('hw_tag');
            } else {
                self.options.dropParam.tag = $('[hw_tag_list] [hw_tag].active').attr('hw_tag');
            }
        ...
        $('[hw_tag]').on('click', function(e) {
            ...
            if ($('[hw_type].active').attr('hw_type') == 'review') {
                $('[hw_tag_list_review] [hw_tag]').removeClass('active');
            } else {
                $('[hw_tag_list] [hw_tag]').removeClass('active');
            }
            $(e.currentTarget).addClass('active');
            self.ui.courseList.html('');
            //self.ui.courseArea.append('<li class="dropload-load"><span class="loading" hw_loading></span>加载中 ....</li>');
    ...
    updateUrl: function () {
        var self = this;
        var url = 'http://' + window.location.host + '/mobile/all?';
        var param = {};
        ...
        if (self.options.dropParam.tag) {
            ...
            if (param.type == 'review') {
                key = '.tagsReview [hw_tag=' + self.options.dropParam.tag + '] span';
            } else {
