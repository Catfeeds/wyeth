...
var car2 = {
	...
	_countdown							: null,										// 倒计时秒数
	_setIntervalTimer					: null,
	_firstClick							: true,										// 首次点击播放按钮
	_questionScroll						: null,										// 提问页面滚动插件
	_questionRefreshState				: false,									// 提问页面刷新标示

	_messageScroll						: null,										// 留言页面滚动插件
	_messageRefreshState				: false,									// 留言页面刷新标示
	...
	audio_addEvent : function(){
		...
		$(car2._audio).on('play',function(){
			...
			$('.btn_audio span').addClass('audio_open');
		})

		// 声音关闭事件
		$(car2._audio).on('pause',function(){
			$('.btn_audio span').removeClass('audio_open');
		})
	...
	formatSeconds: function(value){
	    ...
	    $('.top-state .minute-decade').html(timeArr[0]);
	    $('.top-state .minute-units').html(timeArr[1]);
	    $('.top-state .second-decade').html(timeArr[2]);
	    $('.top-state .second-units').html(timeArr[3]);
	    // return timeArr;
	},
	// 倒计时开始
	timerStart: function(){
		car2._setIntervalTimer = setInterval(function(){
		    if(--car2._countdown >= 0){
		        car2.formatSeconds(car2._countdown);
		    }else{
		        clearInterval(car2._setIntervalTimer);
		        car2.timerOver();
		    }
		}, 1000);
	},
	...
	// 提问页面获取信息
	geQuestion: function(){
		// car2._questionScroll.refresh();
		console.log('动态加载');
		$('.page-question .tip-upload').addClass('f-hide');
	},
	// 提问页面刷新
	...
	// 提问页面滑动底部动态加载
	// scrollLoadingQuestion: function(){
	// 	if(car2._questionScroll.y == 0 && car2._questionRefreshState){
	// 		car2.refreshQuestion();
	// 		car2._questionRefreshState = false;
	// 		$('.page-question .tip-refresh').addClass('f-hide');
	// 	}
	// 	if(car2._questionScroll.y - car2._questionScroll.maxScrollY == 0){
	// 		car2.geQuestion();
	// 	}
	// },
	scrollMoveQuestion: function(){
		if(car2._questionScroll.y > 40){
			car2._questionRefreshState  = true;
			$('.page-question .tip-refresh').removeClass('f-hide');
		}
		if(car2._questionScroll.y - car2._questionScroll.maxScrollY < -40){
			$('.page-question .tip-upload').removeClass('f-hide');
		}
	},
	...
	geMessage: function(){
		...
		console.log('动态加载');
		$('.page-message .tip-upload').addClass('f-hide');
	},
	// 留言页面刷新
	...
	// 留言页面滑动底部动态加载
	scrollLoadingMessage: function(){
		if(car2._messageScroll.y == 0 && car2._messageRefreshState){
			car2.refreshMessage();
			car2._messageRefreshState = false;
			$('.page-message .tip-refresh').addClass('f-hide');
		}
		if(car2._messageScroll.y - car2._messageScroll.maxScrollY == 0){
			car2.geMessage();
		}
	},
	scrollMoveMessage: function(){
		if(car2._messageScroll.y > 40){
			car2._messageRefreshState  = true;
			$('.page-message .tip-refresh').removeClass('f-hide');
		}
		if(car2._messageScroll.y - car2._messageScroll.maxScrollY < -40){
			$('.page-message .tip-upload').removeClass('f-hide');
		}
	},
	// 将表情代码替换并显示
	...
	init : function(){
		...
		$('<img />').attr('src',$('.page-index .top-state').find('.page-con').attr('data-src'));
		$('<img />').attr('src',$('.page-question').find('.page-con').attr('data-src'));
		$('<img />').attr('src',$('.page-message').find('.page-con').attr('data-src'));

		// loading执行一次
		var loading_time = new Date().getTime();

		// 初始化总时间,单位秒
		//car2._countdown = 4538;
		//car2.formatSeconds(car2._countdown);
		...
		$(window).on('load',function(){
			...
			setTimeout(function(){
				...
				setTimeout(function(){
					...
					// 初始化 vue 写的app
					$(document).trigger('main.loading.done');
				}, 1000);
				...
				// 提问按钮
				$('.button-area .button-question').on(tap,function(){
				...
				/* 提问页 */
				// 关闭按钮
				$('.page-question .close').on(tap,function(){
					$('.page-question').addClass('page-question-disappear');
					setTimeout(function(){
						$('.page-question').addClass('f-hide');
						$('.page-question').removeClass('page-question-appear');
						$('.page-question').removeClass('page-question-disappear');
					}, 800)
				});
