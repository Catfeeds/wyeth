...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var progressSize = $("<td style='display:none'/>");
...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var viewProgess = $('<button style="display:none" class="btn btn-default">查看分块上传进度</button>');
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    if (res.url) {
        ...
        str = "<div style='display:none'><strong>Link:</strong><a href=" + url + " target='_blank' > " + link + "</a></div>" +
            "<div style='display:none' class=hash><strong>Hash:</strong>" + res.hash + "</div>";
    }
    ...
    var imgWrapper = $('<div style="display:none" class="imgWrapper col-md-3"/>');
