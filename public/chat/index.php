<html><head>
  ...
  <script type="text/javascript">
    ...
    function onopen() {
        ...
        var login_data = {
            type: "login",
            cid: "<?php echo isset($_GET['cid']) ? $_GET['cid'] : 1?>",
            user_type: 3,
            author_id: "2"
        };
    ...
    function onmessage(e)
    {
        ...
        switch(data['type']){
            ...
            // 登录 更新用户列表
            case 'login':
                //{"type":"login","client_id":xxx,"name":"xxx","client_list":"[...]","time":"xxx"}
                say(data['author_id'], data['name'],  data['name']+' 加入了聊天室', data['time']);
                console.log(data['name']+"登录成功");
                break;
            // 发言
            case 'say':
                //{"type":"say","from_client_id":xxx,"to_client_id":"all/client_id","content":"xxx","time":"xxx"}
                say(data['from_client_id'], data['from_name'], data['content'], data['time']);
                break;
            // 用户退出 更新用户列表
            case 'logout':
                //{"type":"logout","client_id":xxx,"time":"xxx"}
                say(data['from_client_id'], data['from_name'], data['from_name']+' 退出了', data['time']);
    ...
    function onSubmit() {
        ...
        var to_name = $("#client_list option:selected").text();
        var data = {
            type : "say",
            cid: "<?php echo isset($_GET['cid']) ? $_GET['cid'] : 1?>",
            user_type: 3,
    ...
    // 发言
    function say(from_client_id, from_name, content, time){
        $("#dialog").append('<div class="speech_item"><img src="http://lorempixel.com/38/38/?'+from_client_id+'" class="user_icon" /> '+from_name+' <br> '+time+'<div style="clear:both;"></div><p class="triangle-isosceles top">'+content+'</p> </div>');
    }
